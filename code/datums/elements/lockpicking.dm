...
/datum/element/lockpickable
	...
	var/list/lockpicks = list(/obj/item/lockpick, /obj/item/melee/touch_attack/lesserknock)
	///wedges able to be used
	var/list/wedges = list(/obj/item/rogueweapon/huntingknife, /obj/item/lockpick, /obj/item/melee/touch_attack/lesserknock) //whem we add more thieves tools check this
...
/atom/movable/screen/movable/snap/lockpicking/proc/mob_detection(atom/source, mob/user, list/examine_list)
	...
	examine_list += span_warning("They are picking the [picking_object]'s lock!")
	...
	if(failing)
		if(break_checking_cooldown <= world.time)
			if(prob(10 - skill_level))
				to_chat(picker, "<span class='notice'>Your lockpick broke!</span>")
...
/obj/proc/picked(mob/living/user, obj/lockpick_used, skill_level, difficulty)
	...
	if(prob(60 - (skill_level * 10)))
		to_chat(user, "<span class='notice'>Your lockpick broke!</span>")
		playsound(loc, 'sound/items/LPBreak.ogg', 100 - (15 * skill_level))
		qdel(lockpick_used)
