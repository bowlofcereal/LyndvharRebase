...
/obj/item/clothing/head/roguetown/crown/serpcrown/proc/anti_stall()
	src.visible_message(span_warning("The Crown of Sundmark crumbles to dust, the ashes spiriting away in the direction of the Keep."))
...
/obj/item/clothing/head/roguetown/helmet/heavy/knight/armet
	name = "armet"
	desc = "A close helm of articulated plates, closing tight with the twist of a disc at the base of the neck."
	icon_state = "armet"
...
/obj/item/clothing/head/roguetown/helmet/bascinet/klappvisier
	...
	desc = "A steel bascinet helmet of Eisler design. Bears a full-face visor, or \"klappvisier\", which can greatly reduce visibility."
...
/obj/item/clothing/head/roguetown/helmet/bascinet/klappvisier/AdjustClothes(mob/user)
	if(loc == user)
		playsound(user, "sound/items/visor.ogg", 100, TRUE, -1)
		if(adjustable == CAN_CADJUST)
			adjustable = CADJUSTED
...
/obj/item/clothing/head/roguetown/helmet/heavy/frogmouth
	...
	desc = "A tall and imposing frogmouth-style helm popular in the highest plateaus of the Sundmark. Covers not just the head, but the neck as well. Add a cloth to show the colors of your family or allegiance."
...
/obj/item/clothing/head/roguetown/freihat
	name = "freitrupp plume hat"
	desc = "Fear the mercenary who wears this to battle and lives to collect their pay."
	icon_state = "freihat"
	item_state = "freihat"
	icon = 'icons/roguetown/clothing/head.dmi'
...
/obj/item/clothing/head/roguetown/freihat/attack_right(mob/user)
	..()
	if(!picked)
		var/list/colors = list(
		"Swan White"="#ffffff",
...
/obj/item/clothing/head/roguetown/helmet/heavy/wolfplate
	...
	icon_state = "wolfplate"
	item_state = "wolfplate"
	adjustable = CAN_CADJUST
	emote_environment = 3
	flags_inv = HIDEEARS|HIDEFACE|HIDEHAIR|HIDESNOUT
	flags_cover = HEADCOVERSEYES | HEADCOVERSMOUTH
...
/obj/item/clothing/head/roguetown/helmet/heavy/wolfplate/AdjustClothes(mob/user)
	if(loc == user)
		...
		if(adjustable == CAN_CADJUST)
			adjustable = CADJUSTED
			icon_state = "wolfplate_visor_raised"
			body_parts_covered = HEAD|EARS|HAIR
			flags_inv = HIDEEARS
			flags_cover = null
			if(ishuman(user))
