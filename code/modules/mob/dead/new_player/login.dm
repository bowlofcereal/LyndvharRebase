...
//	winset(client, "outputwindow.output", "max-lines=100")
	...
	if(client)
		if(client.is_new_player())
			to_chat(src, span_userdanger("Due to an invasion of goblins trying to play LYNDVHAR, you need to register your discord account or support us on patreon to join."))
			...
			if(!shown_patreon_level)
				shown_patreon_level = "<font color='#2b5ac0'><b>Lyndhardtian</b></font>"
