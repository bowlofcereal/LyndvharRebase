...
/obj/structure/ascendant_altar/proc/ascend(mob/living/user)
	...
	switch(ascend_stage)
		if(1)
			...
			to_chat(user, span_danger("The first capstone. My mind opens. Anar. Isil. Once one. Then N... S..."))
		...
		if(2)
			to_chat(user, span_danger("The second capstone. V... R... A coin. One, with two faces. Split, is it worthless? Together again, restored? N!"))
			sleep(30)
			to_chat(user, span_userdanger("ANAR - THE SUN BETRAYED??"))
			sleep(20)
			to_chat(user, span_userdanger("ISIL - THE MOON ENCIRCLED??"))
			sleep(10)
			to_chat(user, span_userdanger("NO BODY. NO MURDER. NO BETRAYER. ALL LIES."))
			...
			priority_announce("The sky begins to turn quicker - MAJOR ARCANA: THE HANGED MAN, REVERSED", "THEY KNOW", 'sound/villain/dreamer_warning.ogg')
		if(3)
			to_chat(user, span_danger("THIS VISION! NSRVN? WHAT IS UNWRITTEN?!"))
			user.flash_fullscreen("redflash3")
			user.emote("agony", forced = TRUE)
			sleep(20)
			to_chat(user, span_userdanger("What was Known. What can be Known. Do I know NOTHING. DO I KNOW ALL?"))
			sleep(50)
			to_chat(user, span_userdanger("THOUGHT MAKES ALL. THE MIRROR MAKES REFLECTIONS. THE EYE MAKES ILLUSIONS. THE MIND MAKES TRUTH."))
			sleep(30)
			to_chat(user, span_userdanger("NSRVN! Isil, mother of Mystery! Take hold of my body! Our pain!"))
			user.flash_fullscreen("redflash3")
			user.emote("agony", forced = TRUE)
			user.Stun(30)
			user.Knockdown(30)
			sleep(30)
			to_chat(user, span_userdanger("NSRVN! Anar, father of Wisdom! Take hold of my mind! OUR MIND!"))
			user.flash_fullscreen("redflash3")
			user.emote("agony", forced = TRUE)
			user.Stun(100)
			user.Knockdown(100)
			for(var/i = 1, i <= 10, i++)
				spawn((i - 1) * 5)
					to_chat(user, span_userdanger("SO CLEAR SO CLEAR SO CLEAR SO CLEAR SO CLEAR SO CLEAR SO CLEAR SO CLEAR SO CLEAR SO CLEAR SO CLEAR SO CLEAR SO CLEAR SO CLEAR SO CLEAR SO CLEAR"))
			sleep(30)
			user.flash_fullscreen("redflash3")
...
//all goes dark. tp them over. give them their stats.
			...
			sleep(15 SECONDS)
			to_chat(user, span_mind_control("i muST go O TO THE TRHORne. THE THRONE. THE THRONE. MY KINGDO M. AWAITS. ANAR AND ISIL AWAIT. I MUST D E S C E N D. NSRVN! SO CLEAR!"))
...
/obj/structure/ascendant_altar/proc/heavensaysdanger()
	priority_announce("THE CIRCLE IS BROKEN - MAJOR ARCANA : T$yh3 TOW##ER, RE v3RSED", "ALL IS KNOWN", 'sound/villain/ascendant_intro.ogg')
	sleep(15 SECONDS)
	to_chat(world, span_danger("The ground opens. The sky quakes. NSRVN."))
