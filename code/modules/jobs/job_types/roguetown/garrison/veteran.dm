...
/datum/outfit/job/roguetown/vet/footman/pre_equip(mob/living/carbon/human/H)
	...
	if(H.mind)
		...
		H.mind.adjust_skillrank_up_to(/datum/skill/combat/shields, 4, TRUE) // this is a kinda scary skill to give them, surely it won't go wrong though.
		H.mind.adjust_skillrank_up_to(/datum/skill/combat/wrestling, 4, TRUE)
		H.mind.adjust_skillrank_up_to(/datum/skill/combat/unarmed, 4, TRUE)
		...
		H.mind.adjust_skillrank_up_to(/datum/skill/misc/swimming, 2, TRUE)
		H.mind.adjust_skillrank_up_to(/datum/skill/misc/climbing, 2, TRUE)
		H.mind.adjust_skillrank_up_to(/datum/skill/misc/athletics, 3, TRUE) // two handed weapons require a LOT of stamina.
		H.mind.adjust_skillrank_up_to(/datum/skill/misc/reading, 1, TRUE)
...
/datum/outfit/job/roguetown/vet/calvaryman/pre_equip(mob/living/carbon/human/H)
	...
	if(H.mind)
		...
		if(H.age == AGE_OLD)
			...
			H.mind.adjust_skillrank_up_to(/datum/skill/combat/swords, 1, TRUE)
			H.mind.adjust_skillrank_up_to(/datum/skill/combat/maces, 1, TRUE) // You get a lot of weapon skills, but none are legendary. Jack of all trades, master of none. This is probably worse than just having legendary in one, as people rarely swap weapons mid-combat.
			H.mind.adjust_skillrank_up_to(/datum/skill/combat/axes, 1, TRUE)
...
/datum/advclass/veteran/merc
	...
	tutorial = "You were a sell-sword of a Freitrupp, hard men gambling their lives for pay and glory. You and your comrades threw the dice one time too many; now only you remain, retained by your former employer in gratitude for fighting to the bitter end."
...
/datum/outfit/job/roguetown/vet/merc/pre_equip(mob/living/carbon/human/H)
	neck = /obj/item/clothing/neck/roguetown/gorget
	wrists = /obj/item/clothing/wrists/roguetown/bracers
	shirt = /obj/item/clothing/suit/roguetown/shirt/freishirt // You do NOT get the BLACKSTEEL CUIRASS because yours BROKE & I hate you. Go on a personal quest to replace it or something.
	...
	if(H.mind)
		...
		if(H.age == AGE_OLD)
			...
			H.mind.adjust_skillrank_up_to(/datum/skill/combat/axes, 1, TRUE)
			H.mind.adjust_skillrank_up_to(/datum/skill/combat/polearms, 1, TRUE)
			H.mind.adjust_skillrank_up_to(/datum/skill/misc/athletics, 1, TRUE) // two handed weapons require a LOT of stamina.
	H.verbs |= /mob/proc/haltyell
	...
	switch(weapon_choice)
		...
		if("Halberd")
			...
			H.mind.adjust_skillrank_up_to(/datum/skill/combat/axes, 1, TRUE) // SO, fun fact. The description of the grenzel halbardier says they specialize in axes, but they get no axe skill. Maybe this guy is where that rumor came from.
			H.mind.adjust_skillrank_up_to(/datum/skill/combat/polearms, 1, TRUE)
...
/datum/outfit/job/roguetown/vet/scout/pre_equip(mob/living/carbon/human/H)
	...
	if(H.mind)
		...
		H.mind.adjust_skillrank_up_to(/datum/skill/combat/bows, 4, TRUE) // I very rarely see ranged weapons outside of PVE. Maybe this'll fix that?
		H.mind.adjust_skillrank_up_to(/datum/skill/combat/crossbows, 4, TRUE)
		H.mind.adjust_skillrank_up_to(/datum/skill/combat/whipsflails, 3, TRUE)
		H.mind.adjust_skillrank_up_to(/datum/skill/combat/knives, 3, TRUE)
...
/datum/outfit/job/roguetown/vet/spy/pre_equip(mob/living/carbon/human/H)
	...
	if(H.mind)
		...
		H.change_stat("speed", 1)
		if(H.age == AGE_OLD)
			H.mind.adjust_skillrank_up_to(/datum/skill/combat/knives, 2, TRUE) ///Having Master Knives is extremely negligible for a singular role that isn't even meant to be combative.
			H.mind.adjust_skillrank_up_to(/datum/skill/combat/swords, 1, TRUE)
