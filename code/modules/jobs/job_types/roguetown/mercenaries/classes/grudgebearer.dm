	...
	if(H.mind)
		...
		switch(classchoice)
			if("Hammerbearer")	//Because the armor is race-exclusive for repairs, these guys *should* be able to repair their own guys armor layers. A Dwarf smith isn't guaranteed, after all.
				...
				H.mind.adjust_skillrank_up_to(/datum/skill/craft/armorsmithing, 4, TRUE)	//Shouldn't be better than the smith (though the stats are already)
			...
			if("Axebearer")
				...
				H.mind.adjust_skillrank_up_to(/datum/skill/craft/armorsmithing, 2, TRUE)	//Only here so they'd be able to repair their own armor integrity
				H.mind.adjust_skillrank_up_to(/datum/skill/misc/climbing, 1, TRUE)
				shoes = /obj/item/clothing/shoes/roguetown/boots/armor/dwarven
				cloak = /obj/item/clothing/cloak/forrestercloak/snow
				belt = /obj/item/storage/belt/rogue/leather/black
