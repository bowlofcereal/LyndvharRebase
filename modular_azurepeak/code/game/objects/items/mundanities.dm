...
/obj/item/mundane/puzzlebox/impossible/attack_self(mob/living/user)
	...
	if(do_after(user,100, target = src))
		if((dice_roll) + 4 <= user.STAINT)
			...
			finished_ckeys += ckey
			playsound(src.loc, 'sound/foley/doors/lockrattle.ogg', 75, TRUE)
			to_chat(user, span_notice("As I pop open \the [src], I feel a tingling wave run from my head to my feet. A piece of an ornate crystal tumbles out. When I grab it, it's gone- and I suddenly feel invigorated."))
